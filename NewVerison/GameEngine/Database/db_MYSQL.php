 ...
 * \** --------------------------------------------------- **/
        ...
        class MYSQL_DB {
        	...
        	function register($username, $password, $email, $tribe, $locate, $act, $reflink) {
        		$time = time();
        		...
        		$q = "INSERT INTO " . TB_PREFIX . "users (username,password,access,email,timestamp,tribe,location,act,protect,reflink) VALUES ('$username', '$password', " . USER . ", '$email', $time, $tribe, $locate, '$act', '$timep', '$reflink')";
...
//Choice placement where sector
			function generateBase($sector){
				if ($sector == 0){
					...
					//calculation point start which increases over time
					//COMEMENCE days and month of starting the game
					$day_start = date('j',COMMENCE);
					$month_start = date('n',COMMENCE);
					
					//days, month, hours and minutes now
					$day_now = date('j',time());
					$month_now = date('n',time());
					$hours_now = date('G', time());
					$minutes_now = date('i', time());
					
					//days difference
					$day = $day_now-$day_start;
					//months of difference
					$month = ($month_now - $month_start)*10;
					
					//minimum distance from the coordinates (0|0)
					if ($day<=5){
							$distance=5;
						}
						else{
					...
					//$start= $hours_now+$minutes_now+$distance+$day+$month;
					$start= $hours_now+$distance+$day+$month;
				...
				$query2 = "select id from ".TB_PREFIX."wdata where fieldtype = 3 and occupied = 0 and (x BETWEEN $x_a AND $x_b) and (y BETWEEN $y_a AND $y_b) order by rand() limit 1";
...
//				echo $q;
			...
			*/
			//fix party
			function setCelCp($user, $cp) {
        		$q = "UPDATE " . TB_PREFIX . "vdata set cp = cp + $cp where wref = $user";
        		return mysql_query($q, $this->connection);
        	}
			//end fix
        	...
        	***************************/
        	...
        	function addNotice($uid, $type, $topic, $data, $time = 0,$allyfrom = 0,$allyto = 0,$userally = 0) {
        		if($time == 0) {
        			$time = time();
        		}
        		$q = "INSERT INTO " . TB_PREFIX . "ndata (id, uid, topic, ntype, data, time, viewed,allyto,allyfrom,uaid) values (0,'$uid','$topic',$type,'$data',$time,0,$allyto,$allyfrom,$userally)";
        		return mysql_query($q, $this->connection) or die(mysql_error());
        	}
            ...
            function getallyNotice($uid) {
                $q = "SELECT * FROM " . TB_PREFIX . "ndata where allyto = $uid AND uaid = $uid OR allyfrom = $uid AND uaid = $uid ORDER BY time DESC";
                $result = mysql_query($q, $this->connection);
                return $this->mysql_fetch_all($result);
            }
        	...
        	***************************/
        	...
        	function getResourceOneField($village,$id) {

						$q = "SELECT * FROM ".TB_PREFIX."movement, ".TB_PREFIX."send where ".TB_PREFIX."movement.ref = ".TB_PREFIX."send.id and ".TB_PREFIX."movement.proc = 0 and sort_type = 6 and " . TB_PREFIX . "movement.moveid = $id";
//echo$q;
  //      				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "send where " . TB_PREFIX . "movement.to = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "send.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 6 ORDER BY endtime ASC";
//						echo$q;
        		$result = mysql_query($q, $this->connection);
        		$array = $this->mysql_fetch_all($result);
        		return $array;
        	}
//array(1) { [0]=> array(1) { ["owner"]=> string(1) "6" } } ||array(1) { [0]=> array(1) { ["owner"]=> string(1) "6" } } ||array(1) { [0]=> array(1) { ["owner"]=> string(1) "6" } } ||array(1) { [0]=> array(1) { ["owner"]=> string(1) "6" } } ||
        	function checkMyVillages($wref,$uid) {
				global $session;
        		$q = "SELECT owner from " . TB_PREFIX . "vdata where wref = $wref";
        		$result = mysql_query($q, $this->connection);
        		$data=$this->mysql_fetch_all($result);
				//var_dump($data);echo"||>>$uid<<";
				if ($data[0]['owner']==$uid){
				return true;
				}else{
				return false;
			...
			function checkMyData ($wref){
				$checkOasis=$this->checkMyOasis($wref);
				$checkVillage=$this->checkMyVillage($wref);			
				if ($checkOasis>0 || $checkVillage>0){
				$check=1;
				}else{
				$check=0;
				}
        	...
        	function checkMyOasis($villageid) {
				global $session;


				$q="SELECT * FROM " . TB_PREFIX . "odata WHERE wref=".$villageid." AND owner=".$session->uid;
				$result = mysql_query($q);
				if (mysql_num_rows($result)>0){
					$check=1;
				}else{
        	...
        	function getTotalBounty($units,$att_tribe) {
//var_dump($units);
...
//						echo$i;
						global ${'u'.$i};
//						echo $units['t'.(($i-$start)+1)]."||".${'u'.$i}['cap'];
						$countunit++;
						$max_bounty += $units['t'.$countunit]*${'u'.$i}['cap']; 
			...
			function getReportOfVillage ($uid,$myvillage,$dataarray) {
//				var_dump($dataarray);
				$villageid=$dataarray;
				if ($myvillage == 1){
        			$q = "SELECT * FROM " . TB_PREFIX . "ndata where uid = $uid AND (ntype = 4 OR ntype = 0) AND data LIKE '%$villageid%' ORDER BY time DESC";
				}else{
					$q = "SELECT * FROM " . TB_PREFIX . "ndata where uid = $uid AND ntype != 4 AND data LIKE '%$villageid%' ORDER BY time DESC";
				}
				//$q = "SELECT * FROM " . TB_PREFIX . "ndata where data LIKE '%$villageid%' ORDER BY time DESC";
        		$result = mysql_query($q, $this->connection);
        		return $this->mysql_fetch_all($result);

			}

			function checkMyVillage ($villageid){
				global $session;
				$q="SELECT * FROM " . TB_PREFIX . "vdata WHERE wref=".$villageid." AND owner=".$session->uid;
				$result = mysql_query($q);
				if (mysql_num_rows($result)>0){
					$check=1;
				}else{
        	...
        	function modifyHeroAttack($aid, $unit, $amt) {

        		$q = "UPDATE " . TB_PREFIX . "hero set $unit = $unit - $amt where id = $aid";
        		return mysql_query($q, $this->connection);
        	}

			
/*
			function heroLvlUp($hero_info,$hero_levels,$uid){
//			        $datareturn=array();
					if ($hero_info['experience'] > $hero_levels[$hero_info['level']+1]) { 
						mysql_query("UPDATE ".TB_PREFIX."hero SET `level` = `level` + 1 where `uid`='".$uid."'"); 
						mysql_query("UPDATE ".TB_PREFIX."hero SET `points` = `points` + 5 where `uid`='".$uid."'"); 
//						$hero = mysql_query("SELECT * FROM " . TB_PREFIX . "hero WHERE `uid` = " . $uid . "");
//						$hero_info = mysql_fetch_array($hero);
						$datareturn=true;
//						$datareturn[1]=$hero_info;
					} else {
						$datareturn=false;
//						$datareturn[1]=false;
					}
			...
			function heroMultiLvlUp($hero_info,$hero_levels,$uid){
			        $i=0;
					while ($i<1){
						if (($this->heroLvlUp($hero_info,$hero_levels,$uid)==true){
...
//var_dump($data);
        		for($i = $start; $i <= $end; $i++) {
//				echo '$this-modifyEnforce('.$id.', '.$i.', '.$data['t' . $j . ''].', 1)';
        	...
        	function getNatureArray($vid) {
        		$q = "SELECT u31,u32,u33,u34,u35,u36,u37,u38,u39,u40 from " . TB_PREFIX . "units where vref = $vid";
        		$result = mysql_query($q, $this->connection);
        		return mysql_fetch_assoc($result);
        	}
        ...
        function populateOasis2() {
        	...
        	function populateOasisUnitsLow2() {
				//Fix Polutale
            ...
            function populateOasisUnitsHigh2() {
...
//function winner End Game
public function isWinner()
{
    global $database, $technology;
    $q = mysql_query("SELECT vref FROM ".TB_PREFIX."fdata WHERE f99 = '100'");
    $isThere = mysql_result($q, 0);
    if($isThere > 1)
    {
        header('Location: '.HOMEPAGE.'s2/winner.php');
    }
    else
    {
        ## there is no winner
