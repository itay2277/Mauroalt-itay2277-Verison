...
class Account {
	...
	private function Signup() {
		...
		if($form->returnErrors() > 0) {
			...
			//$uid = $database->activate($_POST['name'],md5($_POST['pw']),$_POST['email'],$_POST['vid'],$_POST['kid'],$act,$act2);
				...
				$uid = $database->register($_POST['name'],md5($_POST['pw']),$_POST['email'],$_POST['vid'],$_POST['kid'],$act,$_POST['reflink']);
				if($uid) {
	...
	private function Activate() {
		...
		if($dbarray['act'] == $_POST['id']) {
			...
			$uid = $database->register($dbarray['username'],$dbarray['password'],$dbarray['email'],$dbarray['tribe'],$dbarray['location'],"",$dbarray['reflink']);
			if($uid) {
	...
	function generateBase($kid,$uid,$username) {
		...
		$database->updateUserField($uid, "gold", A_GOLD, 1); //gold di ingresso
