...
include_once("GameEngine/config.php");
        	...
        	public $note;
        	public $inbox, $sent, $reading, $reply, $archived, $noticearray, $allynoticearray, $noticevillray, $readingNotice = array();
        	private $totalMessage, $totalNotice, $totalNoticeVillray;
        	private $allNotice, $allNoticeVillray = array();
            private $allallyNotice = array();

        	function Message() {
        		$this->getMessages();
        		$this->getNotice();
                $this->getAllyNotice($_GET);
        	...
        	public function noticeType($get) {
                ...
                if(isset($get['s'])) {
                    if($get['s'] == 3) {
                             if($get['f'] == 31)
                             {
                            $typeally = array(1, 2, 3);
                            } else if($get['f'] == 32)
                        {
                            $typeally = array(4, 5, 6, 7);
                        }else{
                            $typeally = array(1, 2, 3, 4, 5, 6, 7);
                        }

                    }

                    if (!is_array($typeally)) { $typeally = array($typeally); }
                    $this->allynoticearray = $this->filter_by_value($database->getallyNotice($session->alliance), "ntype", $typeally);
                }
        		if(isset($get['id'])) {
            ...
            private function getAllyNotice($get) {
                global $database, $session;
               if(isset($get['s'])) {
                    if($get['s'] == 3) {
                             if($get['f'] == 31)
                ...
                $this->allallyNotice = $database->getallyNotice($session->alliance);
                //$this->allynoticearray = $this->filter_by_value($this->allallyNotice, "ntype", $typeally);
                $this->totalNotice = count($this->allallyNotice);
            }

        	function getNoticeVillage($myvillage,$dataarray) {
        		global $database, $session;
        		$this->allNoticeVillray = $database->getReportOfVillage($session->uid,$myvillage,$dataarray);
        		$this->noticeVillray = $this->filter_by_value_except($this->allNoticeVillray, "ntype", 9);
//        		$this->totalNoticeVillray = count($this->allNoticeVillray);
				$i=0;
				$dataray=array();
				foreach ($this->noticeVillray as $data){
					$noticeArray = explode(",",$data['data']);
//					var_dump($noticeArray);
					if ($myvillage==1){
//						if ($data['ntype']==0 and $noticeArray[29]==$dataarray){$dataray[$i]=$data;}elseif ($data['ntype']!=0 and $noticeArray[29]==$dataarray){}
						if ($noticeArray[29]==$dataarray){$dataray[$i]=$data;$i++;}
					}else{
//						if ($data['ntype']==0 and $noticeArray[1]==$dataarray){$dataray[$i]=$data;}
						if ($noticeArray[29]==$dataarray){$dataray[$i]=$data;$i++;}
					}
				}
//				var_dump($dataray);
...
*/
	//fix messaggi ally
        ...
        if($permission[opt7]==1){
			while ($allmembers = mysql_fetch_array($allmembersQ)) {
			$database->sendMessage($allmembers[id],$session->uid,$topic,$text,0);
			//$database->sendMessage($allmembers[id],$session->uid,$topic,$text,1);
			}
        }
    }
	//end fix
