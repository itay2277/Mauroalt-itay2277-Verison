...
if ($process['oasisvid']){
$basearray = $database->getMInfo($process['oasisvid']);
if($basearray['oasistype'] > 0) {
	$basearray = $database->getOMInfo($process['oasisvid']);
}
}
// Temp 
...
//var_dump($process);
//echo"$scout}}}}}}}}}}";
 if ((!isset($process['t4']) || $process['t4'] == '') || $_POST['c']==2){  if (!$process['t4']){$process['t4']=0;}$t4=$process['t4']; }else{  $t4=$process['t4']; if ($session->tribe == 1 || $session->tribe == 2 || $session->tribe == 4 || $session->tribe == 5){ $scout=1;} }  
//echo "$scout";
            ...
            <table id="short_info" cellpadding="1" cellspacing="1"> 
                ...
                <tbody> 
                    ...
                    <tr> 
                        ...
                        <td><a href="karte.php?z=<?php echo$zid; ?>"><?php echo $process[1]; ?> (<?php echo $coor['x']; ?>|<?php echo $coor['y']; ?>)</a></td> 
...
if ($process['oasisvid']){$checkoasis=$database->getOasisV($process['oasisvid']);}else{$checkoasis=0;}
...
?> 
  ...
  <?php           if ($process['oasisvid']<=0){echo'<input class="radio" name="spy" value="2" type="radio">Scout defences and troops                                            </td> ';} ?>
        </tr> 
    </tbody> 
    <?php } ?> 
