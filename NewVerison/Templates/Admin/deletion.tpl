...
$delLink = '<a href="?action=delVil&did='.$varray[$i]['wref'].'&usr='.$user['id'].'" onClick="return del(\'did\','.$varray[$i]['wref'].');"><img src="../img/admin/del.gif" class="del"></a>';
}else{
$capital = '';
$delLink = '<a href="?p=deletion&did='.$varray[$i]['wref'].'&usr='.$user['id'].'"><img src="../img/admin/del.gif" class="del"></a>';
  
}

echo '
    <tr>
        <td><a href="?p=village&did='.$varray[$i]['wref'].'&usr='.$user['id'].'">'.$varray[$i]['name'].'</a> '.$capital.'</td>
...
if(!empty($_GET['did'])) {
    ...
    mysql_query($q);  
    header("Location: admin.php?p=deletion&uid=".$id."");
	//echo"done";
}
?>