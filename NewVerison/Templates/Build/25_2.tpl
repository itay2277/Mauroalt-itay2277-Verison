...
</table><p>Your villages have produced <b><?php echo $database->getVSumField($session->uid, 'cp'); ?></b> points in total. To found or conquer a new village you need <b><?php $mode = CP; $total = count($database->getProfileVillages($session->uid)); echo ${'cp'.$mode}[$total+1]; ?></b> points.</p>
</div>
