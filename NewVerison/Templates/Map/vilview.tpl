...
//There is no
//<br>information available.
//echo $_GET['d']."||||||||";
$dataarray = $_GET['d'];
$myvillage=$database->checkMyVillage($_GET['d']);
$getNoiteVillage=$message->getNoticeVillage($myvillage,$dataarray);
//echo $myvillage;
//var_dump($getNoiteVillage);
      $name = 1;
    for($i=1;$i<=7;$i++) {
    if(count($getNoiteVillage) >= $i) {

        $type = (isset($_GET['t']) && $_GET['t'] == 5)? $getNoiteVillage[$i-1]['archive'] : $getNoiteVillage[$i-1]['ntype'];
      echo "<tr><td><img src=\"img/x.gif\" class=\"iReport iReport$type\" alt=\"".$noticeClass[$type]."\" title=\"".$noticeClass[$type]."\" />";
     echo "<a href=\"berichte.php?id=".$getNoiteVillage[$i-1]['id']."\"> ";
    $date = $generator->procMtime($getNoiteVillage[$i-1]['time']);
	echo $date[0]." ".$date[1]."</a></td></tr>";
        }
        $name++;
    }
    if(count($getNoiteVillage) == 0) {
...
<table cellpadding="1" cellspacing="1" id="options" class="tableNone">
    ...
    ?>
	  ...
	  //$cps = $session->cp;      //fix found new village
      $cps = $database->getVSumField($session->uid, 'cp');
	  // end fix
